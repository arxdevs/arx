...
arx auto-detects the build stack (Gradle/Maven JVM, Node, Python, Go, Rust). A Node package that uses Vite and has no `start` script builds as a static SPA — nginx serves the `dist/` output with an `index.html` fallback. If your repo has a `Dockerfile` at its root, arx uses it as-is. Override per service:
